...
<div id="wrap">
	<?php 
        ...
        $codigo=mysql_query("SELECT catPaquetes.Paquete, catPaquetes.Descripcion, tblPaquetes.Fecha, tblPaquetes.Vigencia, tblPaquetes.Estatus, tblPaquetes.idPaquetes FROM catPaquetes, tblPaquetes  WHERE tblPaquetes.tblClientes_idClientes = '$_SESSION[id]' AND catPaquetes.idPaquete=tblPaquetes.Paquete ORDER BY tblPaquetes.Fecha");
    ?>
    ...
    <!-- begin content -->
        <section id="content" class="container clearfix">
            ...
            <div class="notification-box notification-box-info">
                    <p>Debe contratar un plan para disfrutar todo los beneficios de Yet!.</p>
                            ...
                            if (mysql_num_rows($codigo) > 0) {
                                while ($row=mysql_fetch_array($codigo)) {
                                    ...
                                    if ($row[Estatus]=='0') {
                                        ...
                                        echo '<td>Sin detalle</td>';
                                    }else{
                                        echo '<td>'.$row[Vigencia].'</td>';
                                        echo '<td>Activo</td>';
                                        echo '<td><a class="button green" href="plan.php?id='.$row[idPaquetes].'">Ir a Detalle</a></td>';
                                    }
                                    echo '</tr>';
                                    $cont++;
                                }
